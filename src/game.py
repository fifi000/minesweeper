...
from ClassLibrary.level import Level, Levels
...
class Game:
    ...
    def __init__(self, level: Level):
        ...
        self.best_time_font = pygame.font.SysFont(
            'consolas', best_time_font_size, False
        )
        ...
        self.height = (
            self.sq_size * self.rows
            + (self.rows - 1) * self.line_thickness
            + 2 * self.sq_size
        )
    ...
    @property
    def mines_percent(self):
        return self.level.mines_percent

    @property
    def mines(self):
        return round(self.rows * self.cols * self.mines_percent)
    ...
    def set_level_settings(self):
        match self.level.level:
            ...
            case Levels.Hard:
                self.scale = 1.35
            case Levels.Custom:
                self.scale = 2 if self.rows <= 10 else 21.6 / self.rows
            case _:
    ...
    def draw(self):
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                if (
                    Game.playing_again
                    and square.row == Game.first[0]
                    and square.col == Game.first[1]
                ):
                    self.draw_object(None, square, True, colors.GREEN)
                ...
                if square.clicked and not square.mine:
                    pygame.draw.rect(
                        ...
                        colors.LIGHT_GREY,
                        (
                            square.x + self.line_thickness / 2,
                            square.y + self.line_thickness / 2,
                            self.sq_size,
                            self.sq_size,
                        ),
                    )
                    ...
                    if square.number > 0:
                        text = self.number_font.render(
                            str(square.number),
                            True,
                            self.nums_colors[square.number - 1],
                        )
                    ...
                    self.draw_object(self.bomb, square, True, colors.RED)
                    self.end = True
    ...
    def draw_object(
        self, img, square, draw_square=False, square_color=colors.LIGHT_GREY
    ):
        if draw_square:
            pygame.draw.rect(
                self.win,
    ...
    def custom_init_fun(self):
        ...
        return InputBox(
            self.win, x, y, 3, texts, self.board.level_font, '00,00', gap, 2 * gap
        )
    ...
    def save_best_score(user_score, level_name):
        ...
        if new_top_score:
            # delete duplicates
            scores = sorted(set(scores) | {user_score})[:10]
        ...
        while self.playing:
                ...
                if self.board.level_button.clicked:
                        ...
                        for i, button in enumerate(self.board.levels):
                            if button.collision(mouse_x, mouse_y):
                                ...
                                for j in range(len(self.board.levels)):
                                    self.board.levels[j].color = colors.WHITE

                                self.board.levels[i].color = colors.GRAY
                                break
                        ...
                        for i, button in enumerate(self.custom_input.input_boxes):
                            if button.clicked:
                                if event.key == pygame.K_BACKSPACE:
                                    self.custom_input.input_boxes[i].name = button.name[
                                        :-1
                                    ]
                                elif event.key == pygame.K_TAB:
                                    self.custom_input.input_boxes[i].clicked = False
                                    next_input_box = (i + 1) % len(
                                        self.custom_input.input_boxes
                                    )
                                    self.custom_input.input_boxes[
                                        next_input_box
                                    ].clicked = True
                                elif (
                                    event.unicode in [str(j) for j in range(10)]
                                    and len(button.name) < 2
                                ):
                                    self.custom_input.input_boxes[
                                        i
                                    ].name += event.unicode
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        if self.if_win:
                            ...
                            elif self.winning_box.repeat_button.collision(
                                mouse_x, mouse_y
                            ):
                        ...
                        if self.if_lose:
                            ...
                            elif self.losing_box.repeat_button.collision(
                                mouse_x, mouse_y
                            ):
                                Game.playing_again = True
                                self.new_game = True
                        ...
                        if (
                            not self.end
                            and 0 <= mouse_row < self.rows
                            and 0 <= mouse_col < self.cols
                        ):
                        ...
                        # check if level button is clicked
                            ...
                            self.board.level_button.clicked = (
                                not self.board.level_button.clicked
                            )
            ...
            if self.if_win and not self.if_score_saved:
                self.save_best_score(
                    round(time.time() - self.start_time), self.level_name
                )
...
def main():
    ...
    while playing:
        ...
        if game.new_game:
            ...
            rows = game.custom_input.input_boxes[0].name or 10
            rows = max(5, int(rows))

            cols = game.custom_input.input_boxes[1].name or 10
            cols = max(10, int(cols))

            mines = game.custom_input.input_boxes[2].name or 0.15625
            mines = min(1 / 3, float(mines) / 100)

        level = Level(rows, cols, mines)
